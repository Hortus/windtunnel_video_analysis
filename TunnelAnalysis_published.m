...
for n = 1:nfiles
    ...
    for k = 1:nframes
        ...
        angles(k,1) = k;
        %find where the bending at half height 
        x = (cosd(45)*halfheight);
        ...
        %where half height as hypoteneuse forms a 45 degree angle
        angles(k,3) = f(x);
    ...
    coeffname = char(sprintf('coeff%s.txt',potnum));
